...
- **Smart Suggestions**: Recommend roles based on description
...
- **Assigns token budget** (default from role, customizable)
- Validates token budget ≤ role budget
- Checks available budget in role pool

#### Token Budget Integration
- Each agent receives a token budget upon creation
- Default budget suggested based on role
- Budget must be ≤ role's total budget
- Remaining role budget validated before creation
